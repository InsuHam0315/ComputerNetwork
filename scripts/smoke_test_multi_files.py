"""Run a local smoke test for a ConnectBox multi-file transfer."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start the local server and client, then verify multi-file transfer."
    )
...
def create_source_files(source_dir: Path) -> list[Path]:
    ...
    files[0].write_text("ConnectBox multi-file smoke test\n", encoding="utf-8")
    files[1].write_bytes((bytes(range(256)) * 8)[:1500])
...
def main() -> int:
    ...
    log_dir = project_root / "logs" / f"smoke_multi_{run_id}"
    source_dir = project_root / "testdata" / f"smoke_multi_{run_id}"
    save_dir = project_root / "received" / f"smoke_multi_{run_id}"
    ...
    try:
        source_files = create_source_files(source_dir)
        ...
        for filename, expected_bytes in expected.items():
            received_path = save_dir / filename
        ...
        print("PASS")
        print(f"Verified files: {len(expected)}")
        print(f"Logs: {log_dir.resolve()}")
        return 0
    finally:
